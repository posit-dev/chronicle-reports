...
test_that("card_header_with_chart_downloads has popover with two links", {
  base_path <- create_sample_chronicle_data()
  on.exit(unlink(base_path, recursive = TRUE))
  env <- source_app_env("connect", base_path)
...
test_that("Connect: content hits overview downloads work", {
  base_path <- create_sample_chronicle_data()
  on.exit(unlink(base_path, recursive = TRUE))
  env <- source_app_env("connect", base_path)
  ...
  shiny::testServer(
    wrap_server(
      env$content_hits_overview_server,
      shiny::reactive(hits)
    ),
    {
      session$setInputs(
        content_hits_overview_date_range = date_range,
        content_hits_overview_environment = "All"
      )

      f <- output$download_content_hits_chart
      expect_true(file.exists(f))
      csv <- utils::read.csv(f)
      expect_true(nrow(csv) > 0)

      f <- output$download_content_hits_raw
      expect_true(file.exists(f))

      f <- output$download_content_hits_unique_chart
      expect_true(file.exists(f))
      csv <- utils::read.csv(f)
      expect_true(nrow(csv) > 0)

      f <- output$download_content_hits_unique_raw
      expect_true(file.exists(f))
    }
  )
})

test_that("Connect: content hits by user download works", {
  base_path <- create_sample_chronicle_data()
  on.exit(unlink(base_path, recursive = TRUE))
  env <- source_app_env("connect", base_path)
  ...
  shiny::testServer(
    wrap_server(
      env$content_hits_by_user_server,
      shiny::reactive(hits),
    ...
    {
      session$setInputs(
        content_hits_by_user_date_range = date_range,
        content_hits_by_user_environment = "All"
      )

      f <- output$download_content_hits_by_user
      expect_true(file.exists(f))
      csv <- utils::read.csv(f)
      expect_true(nrow(csv) > 0)
    }
